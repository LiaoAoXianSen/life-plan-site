...
export const useWheelStore = defineStore('wheel', () => {
  ...
  function deleteWheel(id: string) { mutate('delete-wheel', () => { const wheel = byId(id); if (!wheel) return; if (lifePlan.data.wheels.length <= 1) throw new Error('至少保留一个转盘'); tombstone('wheels', id, { name: wheel.name }); wheel.items.forEach(item => tombstone('wheelItems', item.id, { wheelId: id, name: item.name })); lifePlan.data.wheels = lifePlan.data.wheels.filter(item => String(item.id) !== id); }); }
  ...
  function saveLibraryItem(input: { id?: string; name: string; note?: string; tagIds: string[]; weight?: number; enabled?: boolean }) { mutate(input.id ? 'update-wheel-library-item' : 'create-wheel-library-item', () => { const name = String(input.name || '').trim(); const note = String(input.note || '').trim(); const tagIds = validTagIds(input.tagIds); if (!name) throw new Error('请输入公共项名称'); if (!tagIds.length) throw new Error('公共项至少需要选择一个标签'); if (libraryItems.value.some(item => item.id !== input.id && nameKey(item.name) === nameKey(name))) throw new Error('公共项里已有同名内容'); const stamp = getNowLocal(); const existing = libraryItems.value.find(item => item.id === input.id); if (existing) Object.assign(existing, { name, note, tagIds, weight: weight(input.weight), enabled: input.enabled !== false, updatedAt: stamp }); else lifePlan.data.wheelLibraryItems.push({ id: genId(), name, note, tagIds, weight: weight(input.weight), enabled: input.enabled !== false, createdAt: stamp, updatedAt: stamp }); }); }
