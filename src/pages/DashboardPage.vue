...
<template>
  <section class="page active" id="page-dashboard">
    ...
    <div class="today-grid">
      <article class="card"><h2 class="card-title">今日待办</h2><div v-if="todayTodos.length" class="todo-list"><div v-for="todo in todayTodos" :key="todo.id" class="todo-item"><input type="checkbox" :checked="todo.done" :aria-label="`完成 ${todo.text}`" @change="todosStore.toggle(todo.id)" /><button class="todo-text todo-dashboard-link" type="button" @click="router.push({ path: '/todos', query: { todo: todo.id } })">{{ todo.text }}</button></div></div><div v-else class="empty-state">今天没有已安排的待办。</div></article>
      <article class="card"><h2 class="card-title">无截止待办池</h2><div v-if="floatingTodos.length" class="todo-list"><div v-for="todo in floatingTodos" :key="todo.id" class="todo-item"><input type="checkbox" :checked="todo.done" :aria-label="`完成 ${todo.text}`" @change="todosStore.toggle(todo.id)" /><button class="todo-text todo-dashboard-link" type="button" @click="router.push({ path: '/todos', query: { todo: todo.id } })">{{ todo.text }}</button></div></div><div v-else class="empty-state">待办池是空的。</div></article>
