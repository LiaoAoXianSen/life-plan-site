...
export interface ScheduleItem {
  key: string; id: string; sourceType: 'record' | 'todo-session' | 'habit'; type: string; date: string;
...
export function buildScheduleItems(data: LifePlanData, startDate: string, endDate: string, keyword = '', typeFilter = 'all'): ScheduleItem[] {
  ...
  const sessions = typeFilter === 'all' || typeFilter === '待办' ? data.todos.flatMap(todo => todo.sessions.filter(session => String(session.date || '') >= startDate && String(session.date || '') <= endDate).map(session => makeTodoItem(todo, session))) : [];
  ...
  return [...records, ...sessions, ...habits].filter(item => !clean || [item.type, item.title, item.preview, item.meta].join(' ').toLowerCase().includes(clean));
}
