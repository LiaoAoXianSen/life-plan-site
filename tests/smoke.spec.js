...
test('wheel library copy is tag-filtered and history can be exported', async ({ page }) => {
    ...
    const chooseLibraryTag = async name => {
        await libraryModal.locator('#wheel-library-tag-filter').click();
        await libraryModal.locator('#wheel-library-tag-options').getByRole('option', { name, exact: true }).click();
    };
    await expect(libraryModal.locator('#wheel-library-tag-filter')).toHaveAttribute('aria-haspopup', 'listbox');
    await libraryModal.locator('#wheel-library-tag-filter').click();
    await expect(libraryModal.locator('#wheel-library-tag-options')).toBeVisible();
    await expect(libraryModal.locator('#wheel-library-tag-options').getByRole('option')).toHaveCount(3);
    await expect(libraryModal.locator('#wheel-library-tag-options').getByRole('option', { name: '全部标签', exact: true })).toHaveAttribute('aria-selected', 'true');
    await page.keyboard.press('Escape');
    await expect(libraryModal.locator('#wheel-library-tag-options')).toBeHidden();
    await expect(libraryModal.locator('#wheel-library-tag-filter')).toBeFocused();
