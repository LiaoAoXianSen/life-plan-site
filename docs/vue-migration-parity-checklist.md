...
| --- | --- | --- | --- | --- |
| Dashboard | Summaries, today focus, command center, active periods, bounded timeline, and entry points into record preview and domain pages. | Command center, today's Todo/habit ratios, active-goal/week-record summaries, today/floating Todo quick-writes (toggle, 今天做, 执行一次), today habit quick check-in/undo, random Material links, active-period links, and bounded recent timeline links. | No open blocker. | Dashboard cards reflect the same persisted data, support legacy Todo/Habit quick-writes through `lifePlanData` plus local mirrors, and navigate to migrated workflows without data loss. |
| Todos | Full CRUD, urgency/focus sorting, sub-todos, sessions, detail links, idea/record relationships, mirror rebuild. | Create/list/filter/toggle/delete plus inline detail and relationship editing, subtask completion, execution sessions, legacy filters/date presets, Dashboard/calendar detail entry points, linked-record navigation, mirror rebuild, protected independent remote flows, and guarded existing-file auto-sync. | No open blocker. | Todo writes use `todos-service.js`, preserve tombstones, update linked records, and mirror `todoAppData`. |
| Records | Full editor, auto/manual save, preview, date ranges, templates, structured template fields, linked/exclusive todos, idea fields, list/day/week/month views. | New-record modal with meaningful-input autosave plus persisted editor/preview, manual and 3-second existing-record autosave, close/switch/navigation flush, date ranges, linked/exclusive todos, idea-specific fields, diary AI confirmation writeback, legacy filters/day ordering, list/calendar operation events, six legacy-compatible structured templates, and custom template management. Day view preserves fixed 160px timed event width and hover title. | No open blocker. | Users can create, preview, edit, delete, template, filter, and link records/todos with legacy-compatible `content`, `templateId`, `todoIds`, idea fields, and tombstones. |
| Ideas | Status colors, tags, next action, conversion to todo, conclusion, filters, search. | Special-state/status/tag/search filters, record-owned detail editing, Records/Todo deep links, editable pre-create Todo draft conversion, and compatible linked-Todo open exist. | No open blocker. | Idea status/tag/next/conclusion/todo link flows round-trip through records and todos without changing field names. |
| Materials | Material CRUD, optional titles, source/note/tags, long-copy disclosure, search. | Legacy-compatible create/edit/delete with optional `title`, old-data title fallback, tag drafts/deduplication, keyword/type/tag filters, descending sort, random review, summary cards, read-only `material` detail deep links, and tombstones. | No open blocker. | Material fields and tombstones remain compatible; old rows preserve full content while new titles, summaries, details, and searchable tags behave consistently. |
| Tags/Search | Cross-module search and tag navigation across records, todos, goals, materials, templates, and wheel public items; tag center with idea/material/wheel scopes. | Grouped module search with scope filtering, exact detail/query navigation, template-management and wheel-library/tag entry points, and combined tag center with summaries, search, scopes, previews, and read-only module jumps. | No open blocker. | Search covers legacy modules with matching labels and navigation targets without mutating persisted data. |
| Goals | CRUD/progress/status, detail modal entry points, search/Dashboard deep links, and tombstones. | Detail modal create/edit/delete, `goal=<id>` deep links, Dashboard goal-row restoration, legacy `createDate` and tombstones, and progress/status persistence exist. | No open blocker. | Goal writes keep existing fields, avoid Vue-only timestamp churn, and delete with legacy `manual-delete` tombstones. |
| Habits | Full rule editor, edit/delete, notes, undo,补卡, wallet, multi-currency, rewards/penalties, wishes, milestones, diagnostics, dual-write mirror, protected remote workflows. | Basic rule create/edit/delete, advanced reward/penalty/milestone fields, quick check-in, note/backfill check-ins, note edit, undo-latest check-in, reward/penalty ledger reversals, habit archive/restore soft-state, wish create/archive, wallet redeem with negative ledger, diagnostics summary/top issues, confirmed safe repair with pre-repair snapshot, settle-through-yesterday miss/break penalties, habit/check-in tombstones, dirty-state updates, `habitAppData` local mirror, independent remote preview/apply, existing-file protected upload, session-armed first create, and guarded conditional auto-sync. | No open blocker. | Habit local writes preserve legacy `habits`, `checkins`, `habitPointLedger`, `habitRewards`, tombstones, dirty state, and local-only mirror contracts while keeping `lifePlanData` authoritative. |
| Fitness | Body metrics, exercise library, multi-exercise plans, workout logging, live workout, set timers, plan writeback. | Full-field body metric create/edit/delete, library, multi-exercise plan create/edit, plan-start live workouts, service-backed last-performance suggestions, rest timer controls, explicit finish-time plan writeback, workout history create/edit/delete, manual-delete tombstones, and dirty-state coverage. | No open blocker. | Fitness writes are delegated to `fitness-service.js` and preserve old body metric, plan/workout exercise, set, planned-set, tombstone, and dirty-state contracts. |
| Wheel | Canvas/interaction, normal/tag wheels, public library, batch management, history, JSON/CSV, independent WebDAV sync/conflicts. | CRUD, canvas rendering, click/drag spin entry points, normal and tag two-stage spin, public-library name/note text plus tag AND filtering, accessible custom tag listbox, cross-filter batch selection, local/remote AI tag suggestions constrained to existing tags with failure fallback and editable drafts, labeled management forms, history, Todo conversion, JSON/CSV, independent remote preview/apply, protected upload/create, and guarded conditional auto-sync. | No open blocker. | Wheel writes preserve `wheels`, `wheelTags`, `wheelLibraryItems`, `wheelHistory`, Todo conversion links, dirty state and tombstones; filter/listbox state remains page-local and read-only. |
| Sync | Main WebDAV pull/push, snapshots, merge, tombstones, ETag conditional writes, conflict retry, module-specific remotes. | Main protected upload/import-export, Todo/Wheel/Habit independent preview/apply/conditional upload/create flows, main auto-sync debounce, periodic visible sync, visibility-resume sync, and Todo/Wheel/Habit guarded conditional auto-sync. | No open blocker. | Sync preserves paths, ETags, snapshots, merge behavior, and refuses unsafe overwrites. |
| AI | Suggestions, diary analysis confirmation writeback, idea next action, todo breakdown, local fallback/remote config. | Chat capture multi-destination writeback, today-plan and backlog-triage confirmed Todo writeback with `sourceType: 'ai'`, Records diary analysis, Ideas AI next-action confirmed writeback, Todo breakdown confirmed subtask writeback, and Wheel public-library tag suggestions exist with remote/local generation and editable drafts. | No open blocker in the current AI assistant mode audit. | AI actions write back only after confirmation and preserve existing fields. |
| Import/Export | Complete `lifePlanData` backup, import merge with snapshots, not records-only export. | Complete backup/export path exists, import merge creates before/after snapshots, preserves tombstones, refreshes Todo/Habit mirrors, and marks main sync dirty. | No open blocker. | Import/export round-trips complete data with snapshots, mirror rebuilds, dirty-state updates, and merge semantics intact. |
...
- Persist `lifePlanData.materials[]` with the non-breaking `{ id, title, type, content, tags, source, note, createdAt, updatedAt }` contract; `title` is optional in old data and normalized to an empty string.
- Normalize old material rows with missing titles, string tags, invalid types, missing IDs, and missing timestamps through the repository load path; display falls back to the first non-empty content line without truncating stored content.
...
- Support `#/materials?material=<id>` read-only detail restoration with a separate detail-to-edit action and `#/materials?tag=<tag>` filtering, while invalid material queries are cleaned without writing data.
...
### Dashboard Habit Quick Check-In Slice

Status: verified locally

- Add a Dashboard `今日习惯` card driven by `habitsStore.todayHabits` (archived-safe due rules) with compact 打卡 / 撤销 actions.
- Reuse existing `quickCheckin` and `undoLatestCheckin` write paths so checkins, rewards, reverse-penalties, checkin tombstones, dirty state, and local-only `habitAppData` rebuild remain on the Habits store contract.
- Drive the hero/summary habit ratio from the same due list so archived habits do not inflate counts.
- Title click opens `#/habits?habit=<id>`; keep the read-only Dashboard navigation contract by scoping the habit timeline locator when both the list and timeline show the same habit name.
- Verify Dashboard check-in/undo contracts with Playwright; full Vue smoke is 77/77.
...
- Deeper diagnostics repair actions remain later Habit risks.

### Habit Penalty Settlement Slice

Status: verified locally

- Add `结算昨日扣分` on the Habits diagnostics panel; it settles miss/break penalties from each habit `startDate` through yesterday.
- Miss penalties write `habitPointLedger` rows with `type: 'miss'`, `amount: -penaltyPoints`, `sourceId: ${habitId}:${date}:miss`, and note `未完成「...」` only when due, incomplete, and sourceId is absent.
- Break penalties support `fixed` (`...:break:fixed`) and `stage` (`...:break:stage:${days}`) modes with previous-streak gating, matching legacy idempotent sourceIds.
- No-op settles do not mark dirty or rebuild mirrors; successful settles rebuild local-only `habitAppData` with `remoteUploadEnabled: false` and reason `vue-settle-penalties`.
- Verify miss ledger writes, no today penalties, dirty/mirror contracts, and second-click idempotence with Playwright.
...
- Restore the public-library management surface with page-local name/note text filtering, tag AND composition, selected-visible counts, and multi-item selection.
- Use an accessible custom tag listbox with color indicators, external/Escape close, Arrow/Home/End navigation, and Enter/Space selection; stack the filter toolbar and bottom-anchor the option panel on narrow screens.
...
- No open blocker in the current Sync parity audit. Modular Playwright coverage for main protected upload/import-export and Todo/Wheel/Habit independent preview/apply/conditional upload/create/auto-sync is the replacement-candidate sync acceptance gate.
- Vue artifact packaging uses `scripts/package-vue-dist.ps1` / `npm run package:vue` (dist-only). Do not use `scripts/package-clean.ps1` for Vue.

## Replacement-Candidate Acceptance

Status: **READY** on `migration/vue-app-v1`

`migration/vue-app-v1` is accepted as a replacement candidate for the legacy static app on `master` when all of the following hold:
...
- `stash@{0}` = `wip-materials-page-v1-before-vue-preview-experiment`
