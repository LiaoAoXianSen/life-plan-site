...
| Habits | Full rule editor, edit/delete, notes, undo,补卡, wallet, multi-currency, rewards/penalties, wishes, milestones, diagnostics, dual-write mirror, protected remote workflows. | Basic rule create/edit/delete, quick check-in, note/backfill check-ins, note edit, undo-latest check-in, reward/penalty ledger reversals, habit/check-in tombstones, dirty-state updates, `habitAppData` local mirror, independent remote preview/apply, and existing-file protected upload. | No open blocker in the current local Habit management/correction plus remote preview/apply/upload audit; archive, wallet/reward administration, wishes, diagnostics depth, session-armed first create, auto/resume sync, and broader penalty settlement UX remain later Habit risks. | Habit local writes preserve legacy `habits`, `checkins`, `habitPointLedger`, tombstones, dirty state, and local-only mirror contracts while keeping `lifePlanData` authoritative. |
...
- Session-armed first creation with readback verification.
