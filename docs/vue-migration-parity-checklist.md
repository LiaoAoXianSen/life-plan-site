...
| Materials | Material CRUD, type colors, source/note/tags, search. | Legacy-compatible create/edit/delete, content-only required field, type/tag normalization, keyword/type/tag filters, descending sort, random review, `material`/`tag` deep links, and tombstones. | No open blocker. | Material fields and tombstones remain compatible and searchable. |
...
| Habits | Full rule editor, edit/delete, notes, undo,补卡, wallet, multi-currency, rewards/penalties, wishes, milestones, diagnostics, dual-write mirror, protected remote workflows. | Basic rule create/edit/delete, advanced reward/penalty/milestone fields, quick check-in, note/backfill check-ins, note edit, undo-latest check-in, reward/penalty ledger reversals, habit archive/restore soft-state, wish create/archive, wallet redeem with negative ledger, read-only diagnostics summary/top issues, settle-through-yesterday miss/break penalties, habit/check-in tombstones, dirty-state updates, `habitAppData` local mirror, independent remote preview/apply, existing-file protected upload, session-armed first create, and guarded conditional auto-sync. | No open blocker; deeper diagnostics repair remains optional follow-up. | Habit local writes preserve legacy `habits`, `checkins`, `habitPointLedger`, `habitRewards`, tombstones, dirty state, and local-only mirror contracts while keeping `lifePlanData` authoritative. |
...
| Wheel | Canvas/interaction, normal/tag wheels, public library, batch management, history, JSON/CSV, independent WebDAV sync/conflicts. | CRUD, canvas rendering, click/drag spin entry points, normal and tag two-stage spin, public-library tag filtering and batch tag/enable/delete actions, local/remote AI tag suggestions constrained to existing tags with failure fallback and editable drafts, labeled management forms, history, Todo conversion, JSON/CSV, independent remote preview/apply, protected upload/create, and guarded conditional auto-sync. | No open blocker. | Wheel writes preserve `wheels`, `wheelTags`, `wheelLibraryItems`, `wheelHistory`, Todo conversion links, dirty state, tombstones, and old `wheel-tool.js` interaction expectations. |
...
3. Sync acceptance is satisfied by the existing modular Playwright coverage for main protected upload/import-export and Todo/Wheel/Habit independent preview/apply/conditional upload/create/auto-sync contracts (persisted shape, not route smoke alone). Current full Vue smoke is 216/216.
...
5. The remaining explicit non-blocker is deeper Habit diagnostics repair / dual-write readiness tooling beyond the existing read-only diagnostics and settle-through-yesterday flow. Wheel public-library AI tag-suggest is now covered by local/remote generation, fallback, existing-tag filtering, editable draft selection, and no-write-before-save regressions.
