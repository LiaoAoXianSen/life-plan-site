...
当前进度：`life-plan-site` 已完成 Phase 1 的 PC 习惯中心入口重排，并完成 Phase 2/3 的只读诊断、映射预览、snapshot JSON 预览与预览指纹展示；尚未进入双写，也尚未写入 `/apps/habit-app/data.json`。
...
- `D:\project\life-plan-site` 已提交并上传 PC 习惯中心 Phase 1、只读诊断、habit-app snapshot JSON 预览和预览指纹。

下一步应该优先做：

1. 继续完善 `life-plan-site` 的本地双写前置测试和写路径清单。
2. 再逐个接入 `toggleCheckin`、`decreaseCheckin`、备注补卡、钱包兑换等本地双写，不直接开启远端同步。
