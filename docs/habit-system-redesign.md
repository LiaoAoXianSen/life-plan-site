...
当前进度：`life-plan-site` 已完成 Phase 1 的 PC 习惯中心入口重排，完成 Phase 2/3 的只读诊断、映射预览、snapshot JSON 预览与预览指纹展示，完成 Phase 4 本地双写第一版与一致性校验，并完成 Phase 5 本地脚手架：`habitAppSyncConfig` / `habitAppSyncState` 已落本地且远端上传默认关闭；仍未真正请求 `/apps/habit-app/data.json`。
...
- `habitSyncConfig` / `habitAppSyncConfig`
- `habitSyncState` / `habitAppSyncState`
- `habitAutoSyncTimer`
...
脚手架状态：已完成本地配置/状态键与诊断展示。

- `remoteUploadEnabled: false`
- `autoSync: false`
- 诊断页显示 path、本地 hash、上次同步时间
...
1. 在 `sync-service.js` 增加 habit snapshot hash/merge 能力，但仍默认不上传。
2. 增加显式开关后的 pull/push 流程，先手动、后 autoSync。
