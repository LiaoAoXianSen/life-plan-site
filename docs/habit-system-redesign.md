...
当前进度：`life-plan-site` 已完成 Phase 1 的 PC 习惯中心入口重排，完成 Phase 2/3 的只读诊断、映射预览、snapshot JSON 预览与预览指纹展示，并完成 Phase 4 前置 readiness / 本地 `habitAppData` 镜像重建；打卡相关写路径 `toggleCheckin` / `decreaseCheckin` / `appendHabitCheckin` 已接入本地全量双写，其它写路径仍 pending，且仍未上传 `/apps/habit-app/data.json`。
...
- 打卡相关 3 条写路径已 `dualWrite: enabled`：`toggleCheckin`、`decreaseCheckin`、`appendHabitCheckin`。
- 当前策略是写成功后全量重建 `localStorage.habitAppData`，不是增量 patch，也不是云端上传。
- 数据无 danger 且部分写路径已接入时状态为 `partial`；全部接入后才是 `ready`；有 danger 时为 `blocked`。
...
1. 继续接入 `saveHabit`、`submitHabitNoteModal` 编辑备注、`redeemHabitReward`、`saveHabitPointAdjust` 等剩余本地双写，仍不上传远端。
