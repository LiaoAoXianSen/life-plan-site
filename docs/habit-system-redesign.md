...
当前进度：`life-plan-site` 已完成 Phase 1 的 PC 习惯中心入口重排，完成 Phase 2/3 的只读诊断、映射预览、snapshot JSON 预览与预览指纹展示，并完成 Phase 4 前置的本地双写写路径清单、readiness 面板与本地 `habitAppData` 镜像重建；尚未把写路径接入自动双写，也尚未上传 `/apps/habit-app/data.json`。
...
- 当前所有写路径 `dualWrite: pending`，不会在打卡/兑换时自动双写。
...
1. 在 `saveData()` 后或关键写路径内接入本地镜像重建/增量双写，优先 `toggleCheckin`、`decreaseCheckin`、`appendHabitCheckin`，仍不上传远端。
