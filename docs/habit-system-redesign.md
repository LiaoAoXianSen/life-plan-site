...
当前进度：`life-plan-site` 已完成 Phase 1 的 PC 习惯中心入口重排，完成 Phase 2/3 的只读诊断、映射预览、snapshot JSON 预览与预览指纹展示，并完成 Phase 4 前置的本地双写写路径清单与 readiness 面板；尚未真正双写任何写路径，也尚未写入 `/apps/habit-app/data.json`。
...
前置状态：已完成第一版 readiness。

- `habit-service.js` 提供 `getHabitDualWritePathInventory()` 与 `buildHabitDualWriteReadiness()`。
- 诊断页新增「本地双写前置」卡片：状态、阻塞项、11 条写路径清单、下一步动作。
- 当前所有写路径 `dualWrite: pending`，`remoteUploadEnabled: false`。
- 数据无 danger 时状态为 `prepared`；有 danger 时为 `blocked`；全部写路径接入后才是 `ready`。
...
- `D:\project\life-plan-site` 已提交 PC 习惯中心 Phase 1、只读诊断、habit-app snapshot JSON 预览、预览指纹，以及 Phase 4 本地双写前置 readiness。

下一步应该优先做：

1. 按写路径清单优先接入 `toggleCheckin`、`decreaseCheckin`、`appendHabitCheckin` 的本地双写，仍不上传远端。
2. 每个写路径接入后补一致性校验：`habitRecords` / `habitLedger` 数量与余额。
