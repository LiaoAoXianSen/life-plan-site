...
- `material=<id>` 深链接改为打开只读详情；搜索结果和标签中心统一使用标题契约。
- 首页“随机复习素材”不再使用旧 `command-row`，改为复用 master 的 compact 素材卡结构：类型/日期、标题、正文 clamp、来源、备注、标签及“展开内容 / 查看详情”操作区。
...
- [x] 搜索和标签中心已切换到新标题契约；首页 compact 卡片结构与 master 对齐
